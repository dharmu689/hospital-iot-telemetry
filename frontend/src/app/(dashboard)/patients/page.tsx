...
import { Search, User, ChevronRight, Edit, Archive, ChevronLeft, Users } from "lucide-react";
          ...
          {loading && patients.length === 0 ? (
            <table className="w-full text-left text-sm text-gray-400">
              ...
              <tbody className="divide-y divide-gray-700/50">
                {[...Array(limit)].map((_, i) => <PatientTableRowSkeleton key={i} />)}
              </tbody>
            </table>
          ) : (
            <div className={`transition-opacity duration-300 ${loading ? "opacity-50 pointer-events-none" : "opacity-100"}`}>
              ...
              {!loading && patients.length === 0 && (
                <div className="text-center py-16 bg-gray-900/20 flex flex-col items-center gap-4">
                  ...
                  <div>
                    <p className="text-gray-400 font-bold">No active patients</p>
                    <p className="text-gray-600 text-sm mt-1">All patients may have been discharged</p>
                  </div>
                </div>
              )}
            </div>
