...
import { Search, Activity, User, ArchiveRestore, Trash2, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function DischargedPage() {
  ...
  const fetchPatients = () => {
    setLoading(true);
    fetch(`/api/patients?page=${page}&limit=${limit}&search=${encodeURIComponent(searchTerm)}&status=discharged`)
      .then(res => res.json())
      .then(result => {
        if (result.data) {
